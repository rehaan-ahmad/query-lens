...
- [ ] Create the following file structure inside `/backend`:
  ```
  /backend
...
- [ ] Copy `BMW_Vehicle_Inventory.csv` into `/data` folder
- [ ] Write `/db/init_db.py` — creates `querylens.db` and the `vehicles` table:
...
- [ ] Write `/db/seed_db.py` — reads CSV and inserts rows using parameterized `INSERT`:
...
- [ ] Run init + seed, verify with sqlite3 CLI:
  ```bash
...
- [ ] Write a `SCHEMA_DESCRIPTION` string (used in Gemini system prompt) that describes the table in plain English — include every column name, data type, and sample values
...
- [ ] Write `/services/sql_executor.py` with a single function: `execute_query(sql: str, params: tuple) -> list[dict]`
...
- [ ] Write `/services/gemini.py` — initialise the Gemini client:
  - Load API key via: `os.environ["GEMINI_API_KEY"]` (fail fast if missing)
  - Use model: `gemini-1.5-flash`
  - Never log the API key
- [ ] Write the `SYSTEM_PROMPT` constant — this is the most critical prompt engineering task:
...
- [ ] Write function: `nl_to_sql(user_query: str) -> dict` with keys: `sql`, `params`, `explanation`
...
- [ ] Write `/services/chart_selector.py` — function: `select_chart(sql: str, result_data: list[dict]) -> str`
...
- [ ] Implement hallucination guard — when Gemini returns `CANNOT_ANSWER`:
...
- [ ] Implement SQL validation layer before execution:
...
- [ ] Initialise FastAPI app in `main.py`:
...
- [ ] `POST /api/query` — the core endpoint:
...
- [ ] `GET /api/query/history` — returns past queries for session:
  - Requires JWT
  - Returns last 20 queries for the `session_id`

### 4.4 Upload Router (/api/upload)

- [ ] `POST /api/upload` — CSV upload endpoint (Bonus Feature):
  - Requires JWT
...
- [ ] `GET /health` — unauthenticated, returns `{ "status": "ok" }` — for uptime monitoring
...
- [ ] Scaffold Next.js 14 app inside `/frontend`:
  ```bash
...
- [ ] Install frontend dependencies:
  ...
  ```
- [ ] Add Google Fonts to `/src/app/layout.tsx`:
...
- [ ] Configure Tailwind with custom color palette in `tailwind.config.ts`:
  ...
  text-muted:    #b2b49c
  ```
- [ ] Create `/src/styles/globals.css` with CSS custom properties for all colors and font assignments
- [ ] Create `/src/lib/api.ts` — centralised API client:
...
- [ ] Build `/dashboard/page.tsx` — three-column layout:
...
- [ ] Build `QueryBar.tsx` — persistent bottom input:
...
- [ ] Build `ResponsePanel.tsx` — houses the 3-act animation:
...
- [ ] Build `Connector.tsx` — animated squares using Anime.js:
...
- [ ] Build `ParticleField.tsx` — ambient particles in response panel during ACT 1:
...
- [ ] Build `ChartRouter.tsx` — renders correct chart from `chart_type` prop:
...
- [ ] Build `BarChart.tsx` using Recharts:
...
- [ ] Build `ScatterChart.tsx` using Recharts:
  - Uses: `ScatterChart`, `Scatter`, `XAxis`, `YAxis`, `ZAxis`, `Tooltip`
- [ ] Build `StatCard.tsx` — single KPI display (no chart):
...
- [ ] Show upload progress and sanitized column names after successful upload

### 6.5 History Page

- [ ] Build `/history/page.tsx`:
...
- [ ] Add follow-up chat to `ResponsePanel.tsx` and history page:
...
- [ ] Test the full pipeline with these 3 progressively complex queries (required by problem statement):
...
- [ ] Test ambiguous/vague query handling:
...
- [ ] Verify correct SQL is generated for each of the 3 demo queries (check query logs)
- [ ] Verify chart type selection matches the query intent for each demo
- [ ] Verify hallucination guard triggers for out-of-scope questions

### 8.3 UX Polish (30-point criterion)

- [ ] Add loading skeleton on chart area while querying (not just a spinner)
- [ ] Ensure Recharts tooltips show formatted numbers (commas, currency symbols where relevant)
- [ ] Test all chart components at mobile viewport (min 375px wide)
- [ ] Verify 3-act animation flow works smoothly end-to-end at normal network speed
- [ ] Add empty-state illustration for dashboard before first query is made
- [ ] Add keyboard shortcut: `Cmd+K` / `Ctrl+K` focuses query bar from anywhere

### 8.4 Presentation Preparation (10-minute demo)

- [ ] Prepare the 3 demo queries (above) to run live — have backup screenshots in case of network issues
- [ ] Prepare a 4th query demonstrating follow-up chat: refine a previous result
- [ ] Prepare a 5th query for CSV upload demo: upload a custom CSV and query it
- [ ] Write a one-paragraph architecture overview explaining the text → LLM → SQL → chart pipeline
- [ ] Be ready to show the Gemini system prompt live — explain your few-shot examples

### 8.5 GitHub Repository Cleanup

- [ ] Write `README.md` with: project description, setup instructions, env var list, architecture diagram (text-based is fine)
- [ ] Run final gitleaks scan before making repo public
- [ ] Ensure `.env.example` is committed and `.env` is NOT in any commit
- [ ] Tag final submission commit: `git tag v1.0.0-submission`
